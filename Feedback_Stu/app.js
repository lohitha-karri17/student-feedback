  ...
  db.query(sql, [username], (err, results) => {
    ...
    // });
      ...
      db.query(rTableSql, [userDetails.roll], (err, rTableResults) => {
        ...
        res.render('student.ejs', { user: req.session.user, userDetails, courses, rTable });
});
});
});});
...
// app.get('/views/notgiven',(req,res)=>{

//   db.query(`SELECT rollno,name from sem31 where status=0`,(err,res1)=>{
//     if(err){
//       throw err;
//     }
//   });
//   db.query(`SELECT rollno,name from sem32 where status=0`,(err,res2)=>{
//     if(err){
//       throw err;
//     }
//   });
//   let res1a=res1;
//   let res2a=res2;
//   res.render('notgiven.ejs',{res1a,res2a});
// });


app.get('/views/notgiven', (req, res) => {
  let sem31Data, sem32Data;

  db.query(`SELECT rollno, name FROM sem31 WHERE status = 0`, (err, res1) => {
    if (err) {
      console.error("Error in sem31 query:", err);
    ...
    db.query(`SELECT rollno, name FROM sem32 WHERE status = 0`, (err, res2) => {
      if (err) {
        console.error("Error in sem32 query:", err);
        return res.status(500).send("Internal Server Error");
      }
      sem32Data = res2;

      res.render('notgiven.ejs', { sem31Data, sem32Data });
    });
  });
});
